	...
	BEGIN TRY
		...
		SELECT
			...
			CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single'
				 ...
				 ELSE 'n/a'
			END cst_marital_status, -- Normalization marital status values to readable format
			CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				 WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				 ELSE 'n/a'
			END cst_gndr, -- Normalization gender values to readable format
			cst_create_date
		FROM (
			SELECT
				*,
		...
		WHERE flag_last = 1; -- Select the most recent recordd per customer
